...
const roomName = 'SPL Pub 1';
const maxPlayers = 30;
const roomPublic = headless ? false : true;
const token = 'token'; // Insert token here
const roomGeo = {"code": "gb", "lat": 50, "lon": 0};
    ...
    if (player.team == Team.SPECTATORS || players.length == 1) {
        if (AFKSet.has(player.id)) {
            if (AFKMinSet.has(player.id)) {
                ...
                room.sendAnnouncement(
                    `⚡️ ${player.name} is not AFK anymore !`,
                    null,
                    announcementColour,
                    'bold',
                    null
...
room.onPlayerJoin = function (player) {
    ...
    if (masterList.findIndex((auth) => auth == player.auth) != -1) {
        ...
        room.sendAnnouncement(
            `⚡️ Admin ${player.name} has joined to the room`,
            null,
            announcementColour,
            'bold',
            HaxNotification.CHAT
...
setInterval(notify.update, 200000);